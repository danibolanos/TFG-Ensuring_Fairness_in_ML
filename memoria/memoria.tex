...
Debido a la gran cantidad de opciones que se nos presentan, nos gustaría saber qué nociones de equidad tienen menos limitaciones en la práctica y qué método de mitigación de sesgo aporta menos complejidad en los algoritmos de predicción. De esta forma podremos conocer las virtudes de cada familia de equidad y seremos capaces de desarrollar nuevos conceptos o adaptar cada uno al caso de estudio que mejores resultados nos proporcione.
...
\begin{definition}[Medida de probabilidad] Una medida de probabilidad $P$ sobre un espacio de medida $(\Omega, \mathcal{A})$ es una función $P\colon \mathcal{A} \to [0,1]$ que verifica:
...
\begin{figure}[h]
	\centering
	\includegraphics[width=9.4cm]{distnormal.png}
...
Una variable aleatoria discreta $X$ con posibles valores $\{x_1,\dots,x_n\}$ diremos que sigue una distribución \textit{uniforme} si: $$P(X=x_i)=\frac{1}{n}, \ \text{para todo} \ i=1,\dots,n.$$
\end{definition}\

\begin{definition}[Distribución uniforme continua]
Una variable aleatoria continua $X$ sigue una distribución \textit{uniforme} en el intervalo $(a,b)$ si su función de densidad viene dada por: $$f(x)=\left \{
\begin{array}{l l}
...
%La inferencia estadística es el proceso de deducción de las propiedades de una distribución mediante el análisis de los datos con los que disponemos que suponemos que provienen de un conjunto de datos más grande que queremos conocer. 
...
Dada una variable aleatoria $X$ que sigue una distribución $F$, una \textit{muestra aleatoria simple} de tamaño $n$, es un conjunto de $n$ variables aleatorias $(X_1,\dots,X_n)$, independientes e idénticamente distribuidas, obtenidas a partir de $X$ y que se distribuyen igual que la misma.
\end{definition}\
...
Una sucesión de variables aleatorias $\{X_n\}$ \textit{converge en probabilidad} a la variable aleatoria $X$ si para todo $\varepsilon>0$ entonces, $$\lim_{n\to +\infty}P([X_n-X]>\varepsilon)=0.$$
...
\begin{example}[Otra definición del algoritmo perceptrón]
...
%Normalmente las aplicaciones que utilizan datos con un  número relativamente pequeño de características suelen usar modelos de predicción lineales. Por otro lado, si los datos de entrenamiento del modelo son imágenes, vídeos, fragmentos de audio o texto, se suelen aplicar modelos no lineales para obtener mejores resultados.
...
Lo más relevante de la representación para este trabajo es conocer que la mecánica de entrenamiento de un modelo sigue siendo la misma, y los detalles de las arquitecturas de las redes neuronales rara vez importan para las cuestiones relativas a la equidad. %Para cualquier afirmación sobre un sistema de aprendizaje automático, hay que estudiar su correspondencia en el caso de modelos lineales simples y si no tiene sentido para estos, probablemente tampoco lo tendrá para los modelos no lineales.
...
Podemos relajar el concepto de paridad demográfica suponiendo que $\hat{Y}=1$ y aproximando su definición con una acotación en valor absoluto de las probabilidades a partir una constante fijada $\varepsilon \in (0,1]$, donde $\varepsilon=\frac{p}{100}$ para que se satisfaga la ''regla $p$'' (\cite{constraints2017}). De esta manera, aproximamos este criterio de equidad como: 

\begin{equation*}
\abs{P(\hat{Y}=1 \mid A=a_r)-P(\hat{Y}=1 \mid A=a_o)} \leq \varepsilon.
\end{equation*}

Tomando un $\varepsilon \in (0,1]$ también podemos aproximar el concepto de paridad demográfica de la siguiente manera:
\begin{equation}\label{eq:p}
   1-\varepsilon \leq \ddfrac{P(\hat{Y}=1 \mid A=a_o)}{P(\hat{Y}=1 \mid A=a_r)}. 
\end{equation}
...
La formulación aportada por Aequitas se basa en calcular la fracción de la métrica de grupo elegida y una vez calculada, comprobar si se encuentra dentro de un rango definido. Tomando $p=\frac{1}{1-\varepsilon}$, obtenemos la acotación que define el rango. La pertenencia o no al rango es una aproximación a la igualación de las métricas en la Definición \ref{def:parmetr}.

\begin{equation}
1-\varepsilon \leq \text{Métrica de disparidad}_{G} \leq \frac{1}{1-\varepsilon}.
\end{equation}

Usamos $\varepsilon \in
(0,1]$ para controlar el rango de valores de disparidad que pueden considerarse justos. Para aplicar la regla del 80\%, simplemente bastaría con tomar $1-\varepsilon=0,8$. Diremos que un clasificador será tan justo como lo permita el valor máximo del sesgo entre los grupos definidos por $A$. 
...
\text{sujeto a:} &\quad P(\hat{Y}=1 \mid A=0, Y=-1)-P(\hat{Y}=1 \mid A=1, Y=-1) \leq \varepsilon,\\
&\quad  P(\hat{Y}=1 \mid A=0, Y=-1)-P(\hat{Y}=1 \mid A=1, Y=-1) \geq -\varepsilon.
\end{split}
\end{equation}
...
\ctparttext{\color{black}\begin{center}
Presentación de los grafos y medidas causales y discusión del concepto de equidad contrafactual para la elaboración de un ejemplo práctico sobre esta noción de justicia.
\end{center}}
...
\begin{itemize}
    ...
    \item \textbf{Nivel 3:} Suponer un modelo totalmente determinista con variables de ruido. Por ejemplo, la distribución $P(V_i \mid pa_i)$ puede tratarse como un modelo de error aditivo, $V_i =f_i(pa_i)+\varepsilon_i$ (\cite{noise2014}). El término de error $\varepsilon_i$ sirve como una entrada a $\hat{Y}$ calculada a partir de las variables observadas. Esto maximizará la información extraída por el clasificador.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{split}
        ...
        \text{LSAT} &\sim Poisson(\text{exp}(b_L++w_L^K K +w_L^R R+w_L^S S)),\\ 
    ...
    \item \textbf{Nivel 3} (\textit{Fair Add}): modelaremos las puntuaciones de GPA, LSAT y FYA como variables continuas con términos de error aditivos independientes de la raza y el sexo. Estimamos los términos de error $\varepsilon_G$, $\varepsilon_L$ ajustando primero dos modelos que utilizan la raza y el sexo para predecir individualmente el GPA y LSAT. A continuación, calculamos los residuos de cada modelo (por ejemplo, $\varepsilon_G =\text{GPA}-\hat{Y}_{\text{GPA}}(R, S)$). Finalmente, usaremos las estimaciones residuales de $\varepsilon_G$, $\varepsilon_L$ para predecir FYA. Este modelo se muestra en la Figura \ref{fig:practicausales}. En este caso las distribuciones vienen dadas por:
    \begin{equation*}
    \begin{split}
        \text{GPA} &= b_G+w_G^R R + w_G^S S +\varepsilon_G, \ \varepsilon_G \sim p(\varepsilon_G)\\
        \text{FYA}&= b_F+w_f^R R + w_F^S S +\varepsilon_F, \ \varepsilon_F \sim p(\varepsilon_F)\\
        \text{LSAT} &= b_L+w_L^R R + w_L^S S +\varepsilon_L, \ \varepsilon_L \sim p(\varepsilon_L).
    \end{split}
    \end{equation*}
\end{itemize}\
...
\ctparttext{\color{black}\begin{center}
Esta es una descripción de las conclusiones
\end{center}}
